...
namespace PotikotTools.Commands
{
    public static class CommandHandler
    {
        ...
        public static void Execute(ICommandInfo commandInfo, object[] parameters)
        {
            if (commandInfo == null || (commandInfo.ParameterTypes.Length > 0 && (parameters == null || commandInfo.ParameterTypes.Length != parameters.Length)))
                return;

            commandInfo.Invoke(parameters);
        }

        public static void Execute(ICommandInfo commandInfo)
        {
            if (commandInfo == null || commandInfo.ParameterTypes.Length > 0)
                return;

            commandInfo.Invoke(null);
        }
